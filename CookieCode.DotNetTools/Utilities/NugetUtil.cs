...
namespace CookieCode.DotNetTools.Utilities
{
    ...
    {
        public static NugetPackageSource[] GetPackageSources()
        {
            var result = GetPackageSources(@"%APPDATA%\NuGet\NuGet.Config");
            return result;
        }

        public static NugetPackageSource[] GetPackageSources(string nugetConfigPath)
        {
        ...
        public static async Task<NuGetVersion[]> FindVersions(NugetPackageSource packageSource, string packageId)
        {
            ...
            SourceRepository repository = Repository.Factory.GetCoreV3(packageSource.Location);
